...
RCT_EXPORT_METHOD(startBackgroundTask:(NSString*)taskName
  ...
  if (identifier == UIBackgroundTaskInvalid) {
    ...
    reject(@"ReactNativeBackgroundService", [NSMutableString stringWithFormat:@"Cannot start background task with id: %@", taskName], nil);
    return;
  }
...
RCT_EXPORT_METHOD(scheduleBackgroundProcessing:(NSString*)taskName
                                       timeout:(nonnull NSNumber*)timeoutSec
                                      resolver:(RCTPromiseResolveBlock)resolve
                                      rejecter:(RCTPromiseRejectBlock)reject) {
  BGProcessingTaskRequest *taskRequest = [[BGProcessingTaskRequest alloc] initWithIdentifier:taskName];
  [taskRequest setEarliestBeginDate:[NSDate dateWithTimeIntervalSinceNow:[timeoutSec doubleValue]]];
  ...
  if ([[BGTaskScheduler sharedScheduler] submitTaskRequest:taskRequest error:nil]) {
    ...
    reject(@"ReactNativeBackgroundService", [NSString stringWithFormat:@"Cannot schedule task: '%@' with timeout: %@", taskName, timeoutSec], nil);
  }
}
